...
def plot_spa_scaling(filename="results_vs_n.csv", quantity="spa"):
    ...
    if quantity == "spa":
        ...
        raise ValueError(
            f"Unknown quantity '{quantity}'. Choose from 'spa', 'error', 'fidelity'."
        )
...
def plot_dissociation_accuracy():
    ...
    legend1 = ax1.legend(
        handles=style_handles, loc="upper right", bbox_to_anchor=(1.0, 1.0)
    )
...
def plot_accuracy_bars(filename="results_all.csv"):
    ...
    # Top: Error
    ax1.bar(
        x - width / 2, df_hf["hf"] - df_hf["fci"], width=width, label="HF", color="navy"
    )
    ax1.bar(
        x + width / 2, df["spa"] - df["fci"], width=width, label="SPA", color="green"
    )
    ax1.legend(loc=2)
