...
def generate_data_point(
    ...
    if get_fci:
        ...
        fci, wfn_fci = mol.compute_energy(
            "fci", get_wfn=True, nroots=nroots, ci0=ci0, use_hcb=True
        )
        fci0 = fci if nroots == 1 else fci[0]
...
def run_dissociation(
    ...
    with open(filename, "w", newline="") as file:
        ...
        for iter in range(max_iter):
            data, data_t = generate_data_point(
                n,
                iter,
                max_iter,
                d_min=d_min,
...
def run_single_point(
    n, distance, nroots=1, get_fci=False, get_hf=False, get_ccsdt=False, verbose=True
):
    """
    :param n: Number of H atoms in H_n chain.
...
def run_scaling(
    ...
    with open(filename, "w", newline="") as file, open(
        filename_t, "w", newline=""
    ) as file_t:
        ...
        for n in range(n_min, n_max + 1, 2):
            ...
            data, data_t = run_single_point(
                n=n, distance=distance, nroots=nroots, get_fci=get_fci, verbose=verbose
            )
            writer.writerow(data)
...
def run_fidelity_spectrum(n, distance, nroots=100):
    ...
    energies, wfns = mol.compute_energy(
        "fci", get_wfn=True, nroots=nroots, ci0=ci0, use_hcb=True
    )
    spectrum = []
    checksum = 0.0
